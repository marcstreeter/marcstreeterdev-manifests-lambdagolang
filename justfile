# Justfile for resume-lambda-go project
...
build:
    cd src && GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w" -o main main.go

# Clean build artifacts
clean:
    rm -f src/main lambda.zip

# Package the Lambda function
package: build
    cd src && zip ../lambda.zip main
...
_with-context: _check-kubectl
    ...
    if ! kubectl config get-contexts {{context}} &> /dev/null; then
        echo "Error: context  is not found. Please check your kubeconfig file."
        exit 1
    fi
    
    if ! kubectl config use-context {{context}}; then
        echo "Error: Failed to switch to context "
        exit 1
    fi
...
setup: _check-gh _check-aws-cli _check-terraform _check-env
    ...
    echo ""
    echo "✅ All required GitHub secrets are now configured:"
